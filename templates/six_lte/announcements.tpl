{foreach from=$announcements item=announcement}
    ...
    <div class="announcement-single">
        <h2 >
            <a class="newstyle" href="{if $seofriendlyurls}{$WEB_ROOT}/announcements/{$announcement.id}/{$announcement.urlfriendlytitle}.html{else}{$smarty.server.PHP_SELF}?id={$announcement.id}{/if}">{$announcement.title}</a>
            <h6 class="showdate">{$announcement.date}</h6>
        </h2>
        <blockquote>
            <p>{if $announcement.text|strip_tags|strlen < 350}{$announcement.text}{else}{$announcement.summary}
            <a href="{if $seofriendlyurls}{$WEB_ROOT}/announcements/{$announcement.id}/{$announcement.urlfriendlytitle}.html{else}{$smarty.server.PHP_SELF}?id={$announcement.id}{/if}" class="label label-success">{$LANG.readmore} &raquo;</a>
            {/if}</p>
        </blockquote>
