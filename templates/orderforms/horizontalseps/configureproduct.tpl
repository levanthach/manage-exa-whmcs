		...
		<div class="panel panel-success">
			...
			<div class="panel-body">
				...
				<form method="post" action="{$smarty.server.PHP_SELF}?a=confproduct&i={$i}">
					...
					<br>
					<p><i class="glyphicon glyphicon-calendar"></i> {$LANG.orderbillingcycle}: {if $pricing.type eq "free"}
					...
					<tr class="ns1 hidden"><td>{$LANG.serverns1prefix}:</td><td><input type="text" name="ns1prefix" size="10" value="ns1"/> {$LANG.serverns1prefixexample}</td></tr>
					<tr class="ns2 hidden"><td>{$LANG.serverns2prefix}:</td><td><input type="text" name="ns2prefix" size="10" value="ns2"/> {$LANG.serverns2prefixexample}</td></tr>
					...
					<p align="center">{if $firstconfig}<input type="submit" class="btn btn-success" value="{$LANG.addtocart}" />{else}<input type="submit" class="btn btn-success" value="{$LANG.updatecart}" />{/if}</p>
